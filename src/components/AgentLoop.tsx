...
export default function AgentLoop() {
  ...
  return (
    <section id="agent-loop" className="relative mx-auto max-w-6xl px-6 py-24">
      <div className="rounded-3xl border border-aurora-violet/30 bg-paper-100/60 p-8 backdrop-blur-md sm:p-12">
              ...
              <motion.div
                ...
                className={`relative flex h-24 w-24 flex-col items-center justify-center rounded-2xl border transition-colors duration-500 lg:h-28 lg:w-28 ${
                  i === idx
                    ...
                    : "border-paper-300/60 bg-paper-100/40 opacity-60"
                }`}
                style={i === idx ? { boxShadow: `0 0 32px -4px ${s.color}66`, borderColor: s.color } : undefined}
              >
                <span className="text-2xl">{s.icon}</span>
                <span className="mt-1 text-xs font-semibold" style={{ color: i === idx ? s.color : "#475569" }}>
                  {s.title}
                </span>
              </motion.div>
              {i < STEPS.length - 1 && (
                <div className="mx-1 hidden text-slate-400 lg:block lg:mt-[-14px]">
                  <svg width="52" height="16" viewBox="0 0 52 16">
                    <line x1="0" y1="8" x2="40" y2="8" stroke={i < idx ? "#7c3aed" : "#cbd5e1"} strokeWidth="2" strokeDasharray="4 4" className={i < idx ? "svg-dash" : ""} />
                    <path d="M 40 3 L 50 8 L 40 13 Z" fill={i < idx ? "#7c3aed" : "#cbd5e1"} />
                  </svg>
                </div>
              )}
            </div>
        ...
        <div className="mt-2 flex justify-center">
          <svg width="300" height="42" viewBox="0 0 300 42" className="max-w-full">
            <path
              ...
              className="svg-dash"
            />
            <path d="M 74 12 L 78 2 L 86 10 Z" fill="#059669" opacity="0.8" />
            <text x="152" y="38" textAnchor="middle" fill="#059669" fontSize="11" opacity="0.8">
              结果回流 → 继续思考（循环直到完成）
            </text>
          </svg>
        </div>
        ...
        {/* 当前步骤详情（终端风格） */}
        <div className="mx-auto mt-8 max-w-2xl overflow-hidden rounded-xl border border-paper-300 bg-paper-50 shadow-card shadow-slate-900/6">
