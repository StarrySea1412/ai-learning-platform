...
const COLOR_MAP = {
  cyan: {
    ...
    glow: "shadow-[0_0_60px_-15px_rgba(34,211,238,0.35)]",
  ...
  blue: {
    ...
    glow: "shadow-[0_0_60px_-15px_rgba(96,165,250,0.35)]",
  ...
  violet: {
    ...
    glow: "shadow-[0_0_60px_-15px_rgba(167,139,250,0.35)]",
  ...
  pink: {
    ...
    glow: "shadow-[0_0_60px_-15px_rgba(244,114,182,0.35)]",
  ...
  amber: {
    ...
    glow: "shadow-[0_0_60px_-15px_rgba(251,191,36,0.35)]",
  ...
  return (
    <section id={concept.id} className="relative mx-auto max-w-6xl px-6 py-24">
      <motion.div
        ...
        className={`rounded-3xl border border-night-700/60 bg-night-900/60 p-8 backdrop-blur-md sm:p-12 ${c.border} ${c.glow}`}
      >
        {/* 头部 */}
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-bold text-white sm:text-4xl">{concept.name}</h2>
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_360px]">
          {/* 左：文字内容 */}
          <div>
            <p className="leading-relaxed text-slate-300">{concept.summary}</p>
            ...
            <div className="mt-8 space-y-5">
              {concept.points.map((p, i) => (
                <motion.div
                  ...
                  className="rounded-xl border border-night-700/50 bg-night-800/40 p-5"
                >
            ...
            <div className={`mt-8 rounded-xl border-l-4 border-aurora-violet/50 bg-aurora-violet/5 p-4`}>
              <p className="text-sm italic leading-relaxed text-slate-300">💡 {concept.analogy}</p>
            </div>
          ...
          {/* 右：SVG 动画图解 */}
          <div className="flex items-center justify-center rounded-2xl border border-night-700/50 bg-night-950/60 p-4">
            ...
            {concept.deep.map((d, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`self-start rounded-xl border bg-night-800/40 transition-colors ${
                    open ? `${c.border} border-night-600` : "border-night-700/50 hover:border-night-600"
                  }`}
                >
                  <button
                  ...
                  >
                    ...
                    <span className="flex-1 text-sm font-semibold text-slate-200">{d.title}</span>
                  ...
                  <AnimatePresence initial={false}>
                    {open && (
                      ...
                      >
                        <p className="overflow-hidden px-4 pb-4 text-sm leading-relaxed text-slate-400">
                          {d.body}
                        </p>
                      </motion.div>
                    )}
          ...
          <div className="mt-10">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              {concept.formulas.map((f) => (
                <div key={f.title} className="rounded-xl border border-night-700/50 bg-night-950/60 p-5">
                  <h4 className={`text-sm font-semibold ${c.text}`}>{f.title}</h4>
                  <p className="mt-3 overflow-x-auto rounded-lg bg-night-900 px-4 py-3 font-mono text-sm text-slate-200 ring-1 ring-white/5">
                    {f.expr}
                  </p>
                  <p className="mt-3 text-xs leading-relaxed text-slate-400">{f.explain}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-10">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border-l-4 border-green-500/60 bg-green-500/5 p-5">
              <p className="text-xs font-bold tracking-wider text-green-400">👍 正方</p>
              <p className="mt-2 text-sm leading-relaxed text-slate-300">{concept.debate.pro}</p>
            </div>
            <div className="rounded-xl border-l-4 border-red-500/60 bg-red-500/5 p-5">
              <p className="text-xs font-bold tracking-wider text-red-400">👎 反方</p>
              <p className="mt-2 text-sm leading-relaxed text-slate-300">{concept.debate.con}</p>
            </div>
          </div>
        </div>
      </motion.div>
